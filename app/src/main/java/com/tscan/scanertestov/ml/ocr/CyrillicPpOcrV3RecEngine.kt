...
/**
 * Описание: распознавание одной строки кириллицы (модель cyrillic_PP-OCRv3_rec).
 */
...
object CyrillicPpOcrV3RecEngine {
    private const val TAG = "CyrillicPpOcrV3Rec"
    private const val ASSET_MODEL = "models/ocr/cyrillic_PP-OCRv3_rec/model.onnx"
    private const val ASSET_DICT = "models/ocr/cyrillic_PP-OCRv3_rec/dict.txt"
    private const val CACHE_MODEL = "cyrillic_ppocrv3_rec_cached.onnx"
        ...
        synchronized(lock) {
            ...
            return try {
                if (!OpenCVLoader.initLocal()) {
                    return "Не удалось инициализировать OpenCV"
                }
                val app = context.applicationContext
                characterTable = buildCharacterTable(app)
                require(characterTable.size == 165) {
                    "Ожидается 165 классов (blank + dict + space), получено ${characterTable.size}"
                }
                ...
                Log.i(TAG, "Сессия OCR: in=$inputName out=$outputName")
                null
            } catch (e: Exception) {
                Log.e(TAG, "Ошибка загрузки OCR", e)
    ...
    fun recognizeLine(matBgr: Mat): Pair<String, Float> {
        val sess = session ?: error("OCR не загружен")
        val inName = inputName ?: error("inputName")
        val outName = outputName ?: error("outputName")
        val table = characterTable

        val (flat, imgW) = PaddleRecPreprocessor.floatChwFromBgrMatChinese(matBgr)
        val imgH = 48
        val nested = nestedArrayFromChw(flat, imgH, imgW)
        val env = ortEnv!!
