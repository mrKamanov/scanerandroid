...
/**
 * Описание: распознавание строки латиницы и цифр (модель en_number_mobile_v2.0).
 */
...
object EnNumberMobileV2PpOcrRecEngine {
    private const val TAG = "EnNumberPpOcrV2Rec"
    private const val ASSET_MODEL = "models/ocr/en_number_mobile_v2.0_rec/model.onnx"
    private const val ASSET_DICT = "models/ocr/en_number_mobile_v2.0_rec/dict.txt"
    private const val CACHE_MODEL = "en_number_mobile_v2_ppocr_rec_cached.onnx"
    ...
    fun ensureLoaded(context: Context): String? {
        ...
        synchronized(lock) {
            ...
            return try {
                ...
                require(characterTable.size == 97) {
                    "Ожидается 97 классов (blank + en_dict + space), получено ${characterTable.size}"
                }
                ...
                Log.i(TAG, "Сессия OCR en_number: in=$inputName out=$outputName classes=${characterTable.size}")
                null
            } catch (e: Exception) {
                Log.e(TAG, "Ошибка загрузки en_number OCR", e)
    ...
    fun recognizeLine(matBgr: Mat): Pair<String, Float> {
        val sess = session ?: error("en_number OCR не загружен")
